...
Mobile UI responsibility: the backend provides APIs, fields, uploaded media URLs generated by the backend/storage provider, status values, and stored records. The mobile app should build and organize the actual screens, visual layouts, guided measurement UI, image zoom interactions, galleries, video players for listing previews, moodboard presentation, and checkout/account prompts.

Media upload policy: customers, designers, and admins should not paste media links into customer-facing forms. User-provided images, voice notes, portfolio media, inspiration images, moodboard images, listing media, and dispute evidence must be uploaded from the user device as `multipart/form-data`. The backend uploads the files and stores the resulting provider URLs internally. Requests that submit pasted `http://` or `https://` media URLs for these fields are rejected with HTTP `400`.
...
| Visual guides, diagrams, optional video instructions | Integrated as backend/app assets | mobile renders UI; users should not paste guide URLs |
...
| Multiple images, zoom, closeups, video previews, before/after, styled looks | Integrated as uploaded media fields | Listing `images` and `media`; mobile uploads files and handles zoom/player UI |
...
- Current customer/designer media support is upload-based, not pasted-link based.
- Mobile should upload files from the device. The backend stores and returns the uploaded provider URL.
